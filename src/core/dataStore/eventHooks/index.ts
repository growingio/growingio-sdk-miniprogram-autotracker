...
class EventHooks implements EventHooksType {
    ...
    if (['onShareTimeline', 'onAddToFavorites'].includes(eventName)) {
      ...
      return function (...args) {
        ...
        if (
          ...
          result = self.currentPage.updateTimelineResult(
            self.growingIO,
            result ?? {}
          );
        }
        if (eventName === 'onAddToFavorites') {
          result = self.currentPage.updateAddFavoritesResult(
            self.growingIO,
            result ?? {}
          );
        }
        ...
        try {
          ...
          if (eventName === 'onShareAppMessage') {
            ...
            if (self.growingIO.vdsConfig.followShare) {
              result = self.currentPage.updateAppMessageResult(
                self.growingIO,
                result ?? {}
              );
            }
  ...
  componentOverriding = (options: any) => {
    ...
    this.supLifeFcs(options.methods, 'page');
    this.objectTraverse(options.methods, fn);
    return options;
  };
  ...
  initEventHooks = () => {
    ...
    // 小程序重写的对象
    this.growingIO.App = niceTry(() => App);
    this.growingIO.Page = niceTry(() => Page);
    this.growingIO.Component = niceTry(() => Component);
    this.growingIO.Behavior = niceTry(() => Behavior);
    // 快应用重写的对象
